 ...
 * @author insset
 */
public class DiscountServiceImplITest {
    ...
    @Test
    public void retrouver_prix_initial_cas_classique() {
        DiscountServiceImpl svc = new DiscountServiceImpl();
        Double original = svc.calculateOriginalPrice(80.0, 20); // 20% -> 100
        assertEquals(100.0, original, 1e-9);
    }

    @Test
    public void retrouver_prix_initial_taux_zero() {
        DiscountServiceImpl svc = new DiscountServiceImpl();
        Double original = svc.calculateOriginalPrice(50.0, 0);
        assertEquals(50.0, original, 1e-9);
    }

    @Test(expected = IllegalArgumentException.class)
    public void retrouver_prix_initial_prix_final_invalide() {
        new DiscountServiceImpl().calculateOriginalPrice(0.0, 10);
    }

    @Test(expected = IllegalArgumentException.class)
    public void retrouver_prix_initial_taux_negatif() {
        new DiscountServiceImpl().calculateOriginalPrice(100.0, -5);
    }

    @Test(expected = IllegalArgumentException.class)
    public void retrouver_prix_initial_taux_100_non_autorise() {
        new DiscountServiceImpl().calculateOriginalPrice(10.0, 100);
    }
